...
if (isset($_GET['id'])) {
    ...
    if ($ordonnance) {
        ...
        $medicamentQuery = $bdd->prepare("
            SELECT m.Name AS MedicamentName, m.Dosage, om.Quantity
            FROM OrdonnanceMedicament om
            JOIN Medicament m ON om.MedicamentID = m.MedicamentID
            WHERE om.OrdonnanceID = :ordonnanceID
        ");
            ...
            <tbody>
                ...
                if (!empty($medicaments)) {
                    foreach ($medicaments as $medicament) {
                        echo "<tr>";
                        echo "<td>" . htmlspecialchars($medicament['MedicamentName']) . "</td>";
                        echo "<td>" . htmlspecialchars($medicament['Dosage']) . "</td>";
                        echo "<td>" . htmlspecialchars($medicament['Quantity']) . "</td>";
                        echo "</tr>";
                    }
                } else {
